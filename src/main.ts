...
function pageFooter(): string {
  return `<footer class="site-footer"><div class="footer-inner">
    <div><a class="brand" href="/" aria-label="TapRead Canvas home"><img src="/assets/brand-mark.svg" width="42" height="42" alt=""><span>TapRead Canvas</span></a><p class="footer-note">Read selected screen or image text aloud on your device.</p><p class="build-note">Built by Param Factory · v1.0.1 · polish-1</p></div>
...
function instrumentMarkup(): string {
  return `<section class="instrument-wrap" id="reader" aria-labelledby="reader-title">
    <div class="section-heading"><div><p class="eyebrow">Web image reader</p><h2 id="reader-title">Read a screenshot or image</h2><p>Choose an image, adjust the selection, then hear the selected text.</p></div><span class="network-state" id="networkState">Ready on this device</span></div>
...
async function renderRoute(focusHeading = false): Promise<void> {
  ...
  if (route === '/' || route === '/demo') {
    ...
    setMetadata(route); app.innerHTML = legalMarkup(route.slice(1) as 'privacy' | 'terms'); setupTheme();
  } else {
    setMetadata('/404'); app.innerHTML = notFoundMarkup(); setupTheme();
  }
...
document.addEventListener('click', (event) => {
  ...
  event.preventDefault(); history.pushState({}, '', `${destination.pathname}${destination.search}${destination.hash}`);
  void renderRoute(true).then(() => destination.hash && document.querySelector(destination.hash)?.scrollIntoView());
});
addEventListener('popstate', () => void renderRoute(true));
void renderRoute();
...
async function startProduct(demo: boolean): Promise<() => void> {
  ...
  function draw(): void {
    ...
    context.restore(); const pct = normalizedSelection(selection, canvas.width, canvas.height); selectionDescription.textContent = `Selection: ${Math.round(pct.width * 100)}% wide by ${Math.round(pct.height * 100)}% high.`;
  }
  ...
  setupNetwork(); setupInstall(); setupDataTools();
...
function setupTheme(): void { const button = document.querySelector<HTMLButtonElement>('#themeButton'); if (!button) return; const stored = localStorage.getItem('tapread-theme'); if (stored === 'light' || stored === 'dark') document.documentElement.dataset.theme = stored; const currentTheme = () => document.documentElement.dataset.theme || (matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light'); button.setAttribute('aria-label', `Use ${currentTheme() === 'dark' ? 'light' : 'dark'} theme`); button.addEventListener('click', () => { const next = currentTheme() === 'dark' ? 'light' : 'dark'; document.documentElement.dataset.theme = next; localStorage.setItem('tapread-theme', next); button.setAttribute('aria-label', `Use ${next === 'dark' ? 'light' : 'dark'} theme`); }); }
function setupNetwork(): void { const update = () => { const el = document.querySelector('#networkState'); if (!el) return; el.textContent = navigator.onLine ? 'Ready on this device' : 'Offline mode'; el.classList.toggle('offline', !navigator.onLine); }; addEventListener('online', update, { once: true }); addEventListener('offline', update, { once: true }); update(); }
function setupInstall(): void { const button = document.querySelector<HTMLButtonElement>('#installButton'); let prompt: BeforeInstallPromptEvent | null = null; addEventListener('beforeinstallprompt', (event) => { event.preventDefault(); prompt = event as BeforeInstallPromptEvent; button?.classList.remove('hidden'); }, { once: true }); button?.addEventListener('click', async () => { await prompt?.prompt(); prompt = null; button.classList.add('hidden'); }); if ('serviceWorker' in navigator) void navigator.serviceWorker.register('/sw.js').then((registration) => { if (registration.waiting && navigator.serviceWorker.controller) showUpdate(registration); registration.addEventListener('updatefound', () => registration.installing?.addEventListener('statechange', () => { if (registration.waiting && navigator.serviceWorker.controller) showUpdate(registration); })); }); }
