  ...
  <main id="main">
    <section class="hero" aria-labelledby="page-title">
      <div class="hero-copy">
        ...
        <p>When text is trapped in a game, stream, image, or custom canvas, load a screenshot, draw one rectangle, and listen. OCR and speech stay on this device.</p>
    ...
    <section class="instrument-wrap" id="reader" aria-labelledby="reader-title">
      <div class="section-heading"><div><p class="eyebrow">The local reader</p><h2 id="reader-title">Tune in to the words</h2><p>Use a screenshot or saved image. Drag directly over the line or paragraph you want.</p></div><span class="network-state" id="networkState">Ready locally</span></div>
    ...
    <section class="how" id="how" aria-labelledby="how-title"><p class="eyebrow">A narrow tool by design</p><h2 id="how-title">Three moves, then back to what mattered</h2><div class="how-grid"><article><h3>Bring the pixels</h3><p>Take an Android screenshot, share it to the installed web app, or choose it from Photos. You can also paste from a desktop clipboard.</p></article><article><h3>Frame only the words</h3><p>A smaller rectangle reduces noise and makes local English OCR faster. The last frame survives a refresh.</p></article><article><h3>Listen or correct</h3><p>TapRead speaks through your device’s own voices. Edit OCR mistakes before replaying without rescanning.</p></article></div></section>
...
function privacyCopy(): string {
  return `<p class="eyebrow">Plain-language policy</p><h1>Privacy</h1><p class="updated">Effective 27 August 2026</p><p>TapRead Canvas is designed so the product does not need to receive your images or recognized text.</p><h2>What stays on your device</h2><p>Images, selected regions, recognized text, speech preferences, supporter history, and your license token are stored in browser storage on your device. OCR runs locally in WebAssembly. Speech uses the voices supplied by your browser or operating system.</p><h2>What leaves your device</h2><p>We do not use analytics, advertising trackers, third-party fonts, or cloud OCR. If you buy or verify a supporter license, your browser contacts the Sociobot billing API. Checkout is operated by Sociobot/Dodo as merchant of record and has its own transaction records. TapRead sends only the license token for verification.</p><h2>Your control</h2><p>You can clear site data in browser settings, export your TapRead data as JSON, or replace it by importing a prior export. Removing the app also removes local data on browsers that couple installed-app and site storage.</p><h2>Contact</h2><p>Questions can be filed in the public project repository linked in the footer.</p>`;
}

function termsCopy(): string {
  return `<p class="eyebrow">Product terms</p><h1>Terms</h1><p class="updated">Effective 27 August 2026</p><p>TapRead Canvas is an assistive utility for reading text from images you are allowed to capture and process.</p><h2>Acceptable use</h2><p>Do not use TapRead to bypass DRM, Android secure-screen protections, copyright controls, or another person’s privacy. Protected screenshots may appear black, and TapRead will not attempt to defeat that protection.</p><h2>Accuracy and safety</h2><p>OCR and text-to-speech can make mistakes. Do not rely on the output for medication, emergency, legal, financial, or other safety-critical decisions without checking the source.</p><h2>Purchase</h2><p>The ₹499 supporter unlock is a one-time license for local history features. Core reading and data export stay free. Sociobot/Dodo is the merchant of record and handles checkout and refunds. A refunded or revoked license stops unlocking supporter features.</p><h2>Warranty</h2><p>The software is provided “as is,” without warranties, to the extent allowed by law. These terms do not limit rights that cannot legally be limited.</p>`;
}
